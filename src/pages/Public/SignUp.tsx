...
export default function SignUp(): JSX.Element {
  ...
  const handleSubmit = async (
    ...
    if (!nameRegex.test(credentials.firstName)) {
      return setError("Invalid name.");
    }

    if (!nameRegex.test(credentials.lastName)) {
      return setError("Invalid name.");
    }
    ...
    var passwordRegex = /^(?=.*[A-Z])(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/;

    if (!passwordRegex.test(credentials.password)) {
      return setError(
        "Password must be atleast 8 characters long and contains atleast 1 uppercase letter and 1 number."
      );
    }
    try {
      setError("");
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-md">
          <img
            ...
            src={logo}
            alt="Webwork"
            onClick={(e) => handleLink(e, "/", history)}
          />
