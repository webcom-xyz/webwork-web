...
export function* handleGetAssignedScorecards(action) {
  try {
    const { data } = yield call(requestGetAssignedScorecards);
    yield put(setAssignedScorecards(data));
  } catch (error) {
    console.log(error);
  }
}
